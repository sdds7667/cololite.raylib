...
namespace Engine {
    ...
    void FixedSizedTextActor::set_text(const std::string &text) {
        m_text = text;
        auto [width, height] = MeasureTextEx(m_font, text.c_str(), m_font_size, 00);
        m_bounding_box = {
            .x = get_position().x - m_anchor.x * width,
            .y = get_position().y - m_anchor.y * height,
            .width = width,
            .height = height
        };
    }
    ...
    void SpriteActor::set_scale(float scale) {
        this->m_scale = scale;
        BoundedBoxActor::set_bounding_box({
            get_position().x - m_anchor.x * static_cast<float>(texture.width) * m_scale,
            get_position().y - m_anchor.y * static_cast<float>(texture.height) * m_scale,
