...
MapBounds MapBounds::from_radius(std::size_t radius) {
  int bounds = static_cast<int>(radius) - 1;
  return {.min_bounds = {.q = -bounds, .r = -bounds},
...
#undef ASSIGN_CORNER_COORD_IF_POSSIBLE
  return {.hex_coord = normalized_coord,
...
MapCoords::MapCoordsIterator MapCoords::MapCoordsIterator::operator++(int) {
  MapCoordsIterator tmp{size, q, r};
  current_coord.q = q;
  current_coord.r = r;
  ++(*this);
  return tmp;
...
MapCoords::MapCoordsIterator MapCoords::begin() {
  return MapCoordsIterator(m_size);
}
MapCoords::MapCoordsIterator MapCoords::end() {
  return MapCoordsIterator(m_size, m_size, 1);
}
